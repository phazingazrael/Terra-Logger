...
interface OrdinaryFrequencies {
	[ordinaryName: string]: number;
}

interface OrdinaryPositions {
	positionsOn?: { [positionName: string]: number };
	positionsOff?: { [positionName: string]: number };
}

interface OrdinaryData {
	[ordinaryName: string]: OrdinaryPositions;
}

interface Ordinaries {
	lined: OrdinaryFrequencies;
	straight: OrdinaryFrequencies;
	data: OrdinaryData;
}
...
// originally called for country or city listed below

// generating country coa
// const coa = COA.generate(null, null, null, cultureType);
// coa.shield = COA.getShield(city.culture, null);

// define ciy coa
// let kinship = 0.25;
// It starts with a base value of 0.25.
// If b is a capital, it increases the value by 0.1.
// If b is a port (but not a capital), it decreases the value by 0.1.
// If b has a different culture than the current state, it decreases the value by 0.25.
// city.coa = COA.generate(countryCOA, kinship, null, capital);
// b.coa.shield = COA.getShield(b.culture, b.state);

/**
 * Generates a Coat of Arms (CoA) based on the parent CoA and given parameters.
 *
 * @param {CoatOfArms} parent - The parent CoA. If null, the generated CoA will be a random one.
 * @param {number} kinship - A number from 0 to 1 defining the kinship between the parent and generated CoA. 0 means no relation, 1 means identical.
 * @param {number} dominion - A number from 0 to 1 defining the dominion between the parent and generated CoA. 0 means no dominion, 1 means full dominion.
 * @param {string} type - The type of CoA to generate. If null, a random type is chosen.
 *
 * @returns {CoatOfArms} The generated CoA.
 */
const generate = (
	parent: TLCoA | null,
	kinship: number,
	...
	if (ordinary) {
		coa.ordinaries = [
			{ ordinary, t: getTincture("charge", usedTinctures, coa.t1) },
		];
		if (linedOrdinary)
			coa.ordinaries[0].line =
				usedPattern || (division && P(0.7)) ? "straight" : rw(lines);
		if (
			division &&
			!addCharge &&
			!usedPattern &&
	...
	if (addCharge) {
		const charge = (() => {
			...
			if (type && type !== "Generic" && P(0.3))
				return rw(typeMapping[type as keyof typeof typeMapping]);
			return selectCharge(
				ordinary || divisioned ? charges.types : charges.single,
			);
		})();
		const chargeData = charges.data[charge as keyof typeof charges.data] || {};

		let p = "e";
		let tinc: string | undefined = "gules";

		const ordinaryData =
			ordinaries.data[ordinary as keyof typeof ordinaries.data];
		const tOrdinary = coa.ordinaries ? coa.ordinaries[0].t : null;

		if ("positionsOn" in ordinaryData && ordinaryData?.positionsOn && P(0.8)) {
			// place charge over ordinary (use tincture of field type)
			p = rw(ordinaryData.positionsOn);
			tinc =
				!usedPattern && P(0.3) ? coa.t1 : getTincture("charge", [], tOrdinary);
		} else if (
			"positionsOff" in ordinaryData &&
			ordinaryData?.positionsOff &&
			P(0.95)
		) {
			...
			tinc = (
				!usedPattern && P(0.3)
					? tOrdinary
					: getTincture("charge", usedTinctures, coa.t1)
			) as string;
		} else if (
			...
			tinc = getTincture(
				"charge",
				tOrdinary ? [...usedTinctures, tOrdinary] : usedTinctures,
				coa.t1,
			);
		} else if ("positions" in chargeData && chargeData.positions) {
			// place charge-suitable position
			p = rw(chargeData.positions);
			tinc = getTincture("charge", usedTinctures, coa.t1);
		} else {
			...
			tinc = getTincture(
				"charge",
				[...usedTinctures, tOrdinary as string],
				coa.t1,
			);
		}

		if (
			"natural" in chargeData &&
			chargeData.natural &&
			chargeData.natural !== tinc &&
			chargeData.natural !== tOrdinary
		)
			tinc = chargeData.natural;

		const item = { charge: charge, t: tinc, p, t2: tinc, t3: tinc };
		const colors = "colors" in chargeData ? chargeData.colors || 1 : 1;
		if (colors > 1)
			item.t2 = P(0.25) ? getTincture("charge", usedTinctures, coa.t1) : tinc;
		if (colors > 2 && item.t2)
			item.t3 = P(0.5) ? getTincture("charge", usedTinctures, coa.t1) : tinc;
		coa.charges = [item];
		...
		if (p === "ABCDEFGHIKL" && P(0.95)) {
			...
			const t2 = getTincture("charge", [], tinc);
			coa.charges.push({ charge, t: t2, p, size: 0.5 });
		} else if (division && !ordinary) {
			const allowCounter =
				!usedPattern && (!coa.line || coa.line === "straight");
			...
			// counterchanged, 40%
				...
				const [p1, p2, p3, p4] =
					division === "perCross" ? ["j", "l", "m", "o"] : ["b", "d", "f", "h"];
				coa.charges[0].p = p1;
		...
		for (const charge of coa.charges) {
			if (charge.charge !== undefined) {
				defineChargeAttributes(ordinary, division, {
					charge: charge.charge ?? "",
					p: charge.p ?? "",
					size: charge.size ?? 1,
					sinister: charge.sinister === true ? 1 : 0,
					reversed: charge.reversed === true ? 1 : 0,
				});
			}
		}
	}
	...
	function selectCharge(set: any) {
		...
		return type === "inescutcheon"
			? "inescutcheon"
			: // biome-ignore lint/suspicious/noExplicitAny: The object is similar in each calling but has slight variations.
				rw((charges as any)[type]);
	}

	// select tincture: element type (field, division, charge), used field tinctures, field type to follow RoT
	// getTincture("division", usedTinctures, P(0.98) ? coa.t1 : null)
	function getTincture(
		element?: string,
		fields?: string[],
		RoT?: string | null,
	) {
		const base: string | null = RoT?.split("-")[1] ?? RoT ?? null;

		let type = rw(tinctures[element as keyof typeof tinctures]); // metals, colours, stains, patterns
		if (RoT && type !== "patterns")
			type = getType(base ?? "") === "metals" ? "colours" : "metals"; // follow RoT
		if (
			type === "metals" &&
			fields?.includes("or") &&
			fields.includes("argent")
		)
			type = "colours"; // exclude metals overuse
		let tincture = rw(tinctures[type as keyof typeof tinctures]);

		while (tincture === base || fields?.includes(tincture)) {
			tincture = rw(tinctures[type as keyof typeof tinctures]);
		} // follow RoT

		if (type !== "patterns" && element !== "charge")
			// biome-ignore lint/suspicious/noExplicitAny: The object is similar in each calling but has slight variations.
			usedTinctures.push(tincture as any); // add field tincture
	...
	function defineChargeAttributes(
		// biome-ignore lint/suspicious/noExplicitAny: The object is similar in each calling but has slight variations.
		ordinary: any,
		// biome-ignore lint/suspicious/noExplicitAny: The object is similar in each calling but has slight variations.
		division: any,
		...
		// define orientation
		if (c.charge && P(0.02) && charges.data[c.charge]?.sinister) c.sinister = 1;
		if (c.charge && P(0.02) && charges.data[c.charge]?.reversed) c.reversed = 1;
	}
	...
	function getType(t: string) {
		...
		if (Object.keys(tinctures.colours ?? {}).includes(tincture))
			return "colours";
		if (Object.keys(tinctures.stains ?? {}).includes(tincture)) return "stains";
	}

	function isSameType(t1: string, t2: string) {
		return type(t1) === type(t2);

		function type(tincture: string) {
			if (Object.keys(tinctures.metals ?? {}).includes(tincture))
				return "metals";
			if (Object.keys(tinctures.colours ?? {}).includes(tincture))
				return "colours";
			if (Object.keys(tinctures.stains ?? {}).includes(tincture))
				return "stains";
			return "pattern";
		}
	}
	...
	// function replaceTincture(t: string, n?: string) {
	// 	const type = getType(t);
	// 	let result = n;
	// 	while (!n || n === t) {
	// 		result = rw(tinctures[type ?? "metals"]);
	// 	}
	// 	return result;
	// }

	function getSize(p: string, o = null, d = null) {
...
//const getShield = (culture: number, state: number) => {
const getShield = () => {
	const emblemShape = document.getElementById("emblemShape");
	const shapeGroup =
		(emblemShape as HTMLSelectElement)?.selectedOptions?.[0]?.parentNode ??
		"Diversiform";
...
const ToString = (coa: object) => JSON.stringify(coa).replaceAll("#", "%23");
const copy = (coa: object) => JSON.parse(JSON.stringify(coa));

export { generate, ToString, copy, getShield, shields };
