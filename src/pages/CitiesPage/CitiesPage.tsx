...
function CitiesPage() {
  ...
  useEffect(() => {
    const initializeDatabase = async () => {
      ...
      let sortedCountries = [...countries].sort((a, b) => a.name > b.name ? 1 : -1);
    ...
    <Container>
      <div className="contentSubHead">
        ...
        <div id="search-filter-container" className="search-filter-container">
          <input
            ...
            type="search"
          />
          <Button variant="outlined" id="filter-all" className="filter-all">All</Button>
          <ButtonGroup className="filter-group country-filter">
          {countriesList.map((country) => (
            <Button key={country._id} id={country._id}>{country.name}</Button>
          ))}
          </ButtonGroup>
        </div>
      </div>
      <div className="contentSubBody">
        <Grid container spacing={2}>
          <Suspense fallback={<Grid item xs={12}><BookLoader/ ></Grid>}>
          {cities.map((entry) => (
